...
class FasterRCNN(chainer.Chain):
    ...
    def __init__(
            ...
            num_classes=21, spatial_scale=0.0625):
    ...
    def __call__(self, x, im_info, gt_boxes=None):
        h = self.trunk(x)
        if self.train:
            rpn_cls_loss, rpn_loss_bbox, rois = self.RPN(h, im_info, gt_boxes)
        else:
            rois = self.RPN(h, im_info, gt_boxes)
        ...
        self.cls_prob = F.softmax(cls_score)
        ...
        if self.train:
            ...
            return self.cls_prob, self.pred_boxes
